...
from JUMIA_INTER.items import (JumiaInterItem, clean_discountrate, 
                     ...
                     get_breadlist, add_brand, get_sellerOrderRate, get_sellerscore, 
                     clean_stock)
from scrapy.loader import ItemLoader


class JumiaUgandaSpider(scrapy.Spider):
    name = "jumia_uganda"
    allowed_domains = ["jumia.ug"]
    start_urls = ["https://www.jumia.ug/"]
    
    cats_ug = ['groceries', 'phones-tablets', 'home-office', 'home-office-appliances', 
                'electronics', 'health-beauty', 'category-fashion-by-jumia', 
                'computing', 'sporting-goods', 'baby-products', 'video-games']
    
    def start_requests(self):
        url = self.start_urls[0]
        for cat in self.cats_ug:
