...
class JumiaEgyptSpider(scrapy.Spider):
    name = "jumia_coteivoire"
    allowed_domains = ["jumia.ci"]
    start_urls = ["https://www.jumia.ci/"]
    
    cats_cot = ['epicerie', 'maison-cuisine-jardin', 
                 'telephone-tablette', 'electronique', 'beaute-hygiene-sante', 'bebe-puericulture', 
                 'jardin-plein-air-ferme-ranch',
                 'ordinateurs-accessoires-informatique', 'fashion-mode-femme', 'fashion-mode-homme']
    
    def start_requests(self):
        url= self.start_urls[0]
        for cat in self.cats_cot:
    ...
    def parse_listing_page(self, response):
        ...
        #pagination
        next_page_link = response.xpath("//a[@aria-label='Page suivante']/@href")
        if next_page_link:
